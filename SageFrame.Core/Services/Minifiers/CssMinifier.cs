...
namespace CssJscriptOptimizer.Minifiers
{
	public static class CssMinifier
	{
        ...
        //public static string RewriteCssImagePath(string content,string moduleName, string appRoot, string imageFolder)
        //{
        //    string pattern = "url\\s*\\(\\s*[\"']?(?<imgPath>\\s*[^\"')]*/)?(?<imgfile>\\s*[^\"')]*)[\"')]?";
        ...
        //    {
        //        string elementPattern = "url\\s*\\(\\s*[\"']?" + match.Groups[1].Value + match.Groups[2].Value+"[\"']?";
        //        content = Regex.Replace(content, elementPattern, "url(" + appRoot + moduleName + "/" + imageFolder + "/" + match.Groups[2].Value);
        //    }
