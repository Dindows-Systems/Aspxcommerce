            ...
            BindBreadCrum: function(data) {
                ...
                $.each(arrPages, function(index, item) {
                    if (item != "") {
                        ...
                        var pageLink = BreadCrum.config.PagePath;
                        if (item === "Admin") {
                            pageLink = pageLink + "/Admin/" + childPages + '.aspx';
                            isadmin = true;
                        }
                        else if (item === "Super-User") {
                            pageLink = pageLink + "/Super-User/" + childPages + '.aspx';
                            isadmin = true;
                        }
                        else {
                            pageLink = pageLink + "/" + childPages + '.aspx';
                        }
                        childPages += "/";
                ...
                if (!isadmin) {
                    html = '<li class="first"><a href="' + DefaultPortalHomePage + '">Home</a></li>' + html;
                }
                $(p.Container).append('<ul>' + html + '</ul>');
                //$(p.Container + ' li:last').remove();
                var lastLink = $(p.Container + ' li:last a').html();
                $(p.Container + ' li:last').addClass("last").html(lastLink);
            }
